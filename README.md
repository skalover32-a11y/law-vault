...
Логины, которым доступен раздел управления учетными записями в портале, задаются через:
...
- Worker удаляет данные в строгом порядке: запись ключа, объект в MinIO с проверкой фактического исчезновения, затем запись файла в БД.
- Использованные и осиротевшие записи ссылок тоже очищаются worker'ом.
- Для практической невосстановимости при физическом доступе к диску одного `DELETE` недостаточно: на SSD, overlayfs и в Postgres нельзя надежно гарантировать перезапись старых блоков.
- Для максимальной защиты храните `APP_MASTER_KEY_B64` вне диска сервера и используйте шифрование диска или отдельный внешний KMS/HSM. Без этого нельзя честно обещать абсолютную невосстановимость после физического изъятия носителя.
